...
public class EnchantmentUtils {
    ...
    public static int calculateLevelFromExp(ItemStack stack) {
        int exp = stack.getOrDefault(EnchantInnovation.EXP_COMPONENT_TYPE,0);
        int[] levelInfo = EnchantmentUtils.calculateLevelAndProgress(exp);
        return levelInfo[0];
    }
    public static void addExp(ItemStack stack , int value){

        int currentLevel = stack.getOrDefault(EnchantInnovation.EXP_COMPONENT_TYPE,0);
        stack.set(EnchantInnovation.EXP_COMPONENT_TYPE, currentLevel + value);
    }
